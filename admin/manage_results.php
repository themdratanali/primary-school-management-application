...
// Allow both admin and teacher to access manage results
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['batch_year'], $_POST['student_id'], $_POST['class_id'], $_POST['marks'], $_POST['result_type'])) {
    ...
    if (count($errors) === 0) {
        ...
        if ($handle === false) {
            ...
            // Save notification for admin actions
            $admin_id = $_SESSION['admin_id'] ?? 0;
            $admin_name = $_SESSION['admin_name'] ?? 'Admin';
            $subject_names_str = implode(', ', $subjectNames);
            $notification_msg = "Admin $admin_name added results for student (Roll: $student_roll) in $class_name for subjects: $subject_names_str";
            $notify_stmt = $conn->prepare("INSERT INTO notifications (type, title, message, created_by) VALUES ('result_added', 'Results Added', ?, ?)");
            $notify_stmt->bind_param("si", $notification_msg, $admin_id);
            $notify_stmt->execute();
            $notify_stmt->close();
        }
    } else {
        ...
        <form method="post">
            <div class="results-layout">
                ...
                <div class="results-section">
                    ...
                    <button type="button" id="exportResultsBtn" class="submit-btn" style="margin-top:10px; background:#177a03;">
                        <i class="fas fa-file-csv"></i> Export Results to Excel (CSV)
                    </button>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#exportResultsBtn').on('click', function() {
                ...
                if (!batchId || !classId || !resultType) {
                    alert('Please select Batch, Class, and Result Type before exporting.');
                    return;
                }

                const url = 'export_results_excel.php?batch_id=' +
                    encodeURIComponent(batchId) +
                    '&class_id=' + encodeURIComponent(classId) +
                    '&result_type=' + encodeURIComponent(resultType);
