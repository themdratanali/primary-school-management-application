...
if ($batch_id > 0 && $class_id > 0 && $student_id > 0) {
    ...
    if ($result_table && $result_table->num_rows) {
        ...
        if ($student_res->num_rows) {
            ...
            // Resolve to a web-accessible URL (mirrors student_profile.php)
            $rawPhoto = $student_data['photo'] ?? '';
            $cleanPhoto = str_replace('../', '', $rawPhoto);
            $photoFs = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $cleanPhoto);
            if ($rawPhoto && file_exists($photoFs)) {
                $student_photo = BASE_URL . '/' . $cleanPhoto;
            } else {
                $student_photo = BASE_URL . '/uploads/students/default-photo.jpg';
            }
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo BASE_URL; ?>/library/css/fee_see.css">
...
<body>
        ...
        <form method="get" class="fee-search-form">
            <div class="search-form-inner">
                <div class="form-group">
                    <label class="form-label"><i class="fas fa-calendar-alt"></i> Academic Batch</label>
    ...
    <script>
        $(function() {
            function loadStudents(batchId, classId, selectedStudentId) {
                var $sel = $('#student_id');
                if (!batchId || !classId) {
                    $sel.html('<option value="">-- Select Student --</option>');
                    return;
                }
                $.post('get_students_by_batch_class', {
                    batch_id: batchId,
                    class_id: classId
                }, function(data) {
                    $sel.html(data);
                    if (selectedStudentId) {
                        $sel.val(selectedStudentId);
                    }
                });
            }
    ...
    <div class="mobile-bottom-nav">
        ...
        <a href="<?php echo ams_student_url('fee_see'); ?>" class="active">
