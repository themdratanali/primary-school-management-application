...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($phone) || empty($email) || empty($gender)) {
        $message = "Please fill all required fields (Name, Gender, Phone, Email).";
    } else {
        ...
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($file_ext, $allowed_ext)) {
                if ($_FILES['photo']['size'] <= 2 * 1024 * 1024) {
                    $upload_dir = ams_upload_dir('teachers/' . $teacher_name_safe);
                    $photo = 'uploads/teachers/' . $teacher_name_safe . '/' . uniqid('teacher_', true) . '.' . $file_ext;
                    if (!move_uploaded_file($_FILES['photo']['tmp_name'], $upload_dir . basename($photo))) {
                        ...
                        if ($_FILES['documents']['error'][$i] === UPLOAD_ERR_OK) {
                            ...
                            if (in_array($file_ext, $allowed_ext) && $_FILES['documents']['size'][$i] <= 2 * 1024 * 1024) {
                                ...
                                if (move_uploaded_file($_FILES['documents']['tmp_name'][$i], $doc_full_path)) {
                                    $doc_stmt->bind_param("iss", $teacher_id, $_FILES['documents']['name'][$i], $doc_path);
                                    $doc_stmt->execute();
                                }
                            }
                        }
