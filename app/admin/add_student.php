...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    if (!empty($_FILES['photo']['name']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($ext, ['jpg', 'jpeg', 'png'])) {
            ...
            $student_name_safe = preg_replace('/[^A-Za-z0-9\-]/', '-', $name ?: 'student');
            $folder_name = $batch_name_safe . '-' . $class_name_safe . '/' . $roll . '-' . $student_name_safe;
            $upload_dir = ams_upload_dir('students/' . $folder_name);
            
            $photo_base = rtrim($upload_dir, '/\\') . DIRECTORY_SEPARATOR . $student_name_safe;
            ...
            $photo = 'uploads/students/' . $folder_name . '/' . basename($photo_file);
    ...
    if (empty($errors)) {
        ...
        $batch_name_table = strtolower(str_replace(' ', '_', $batch_name));
        $class_name_table = strtolower(str_replace(' ', '_', $class_name));
        $table_name = "Student_{$batch_name_table}_{$class_name_table}";
        $conn->query("CREATE TABLE IF NOT EXISTS `$table_name` LIKE students");
        
        // Remove foreign keys from dynamic student table
        $conn->query("ALTER TABLE `$table_name` DROP FOREIGN KEY IF EXISTS `{$table_name}_ibfk_1`");
        $conn->query("ALTER TABLE `$table_name` DROP FOREIGN KEY IF EXISTS `{$table_name}_ibfk_2`");
...
// Upload documents
            if (!empty($_FILES['documents']['name'][0])) {
                ...
                $student_name_safe = preg_replace('/[^A-Za-z0-9\-]/', '-', $name ?: 'student');
                $folder_name = $batch_name_safe . '-' . $class_name_safe . '/' . $roll . '-' . $student_name_safe;
                $upload_base = ams_upload_dir('students/' . $folder_name . '/');
                $doc_stmt = $conn->prepare("INSERT INTO students_documents (student_id, file_name, file_path) VALUES (?, ?, ?)");
                for ($i = 0; $i < count($_FILES['documents']['name']); $i++) {
                    if ($_FILES['documents']['error'][$i] === UPLOAD_ERR_OK) {
                        ...
                        if (in_array($file_ext, $allowed_ext) && $_FILES['documents']['size'][$i] <= 2 * 1024 * 1024) {
                            ...
                            $doc_path = 'uploads/students/' . $folder_name . '/' . $doc_filename;
                            $doc_full_path = $upload_base . $doc_filename;
                            if (move_uploaded_file($_FILES['documents']['tmp_name'][$i], $doc_full_path)) {
                                $doc_stmt->bind_param("iss", $student_id, $_FILES['documents']['name'][$i], $doc_path);
                                $doc_stmt->execute();
                            }
                        }
                    }
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo BASE_URL; ?>/library/css/add_student.css">
</head>
<body>
<div class="container">
    ...
    <form method="post" enctype="multipart/form-data" novalidate>
        ...
        <fieldset>
            <legend>Photo</legend>
            <?php if (!empty($photo)): 
                $fsPath = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $photo);
                if (file_exists($fsPath)):
            ?>
                <div>
                    <img src="<?= htmlspecialchars(BASE_URL . '/' . $photo) ?>" alt="Student Photo" style="max-width:150px; display:block; margin-bottom:5px;">
                </div>
            <?php endif; endif; ?>
    ...
    <script>
        $(document).ready(function() {
            function fetchNextRoll() {
                ...
                if (batchId && classId) {
$.ajax({
                         url: '<?= ams_admin_url('get_next_roll') ?>',
                         type: 'POST',
data: { 
                              batch_id: batchId, 
                              class_id: classId,
                        ...
                        success: function(response) { 
                            let rollNum = parseInt(response);
                            if (!isNaN(rollNum)) {
                                $('#roll').val(rollNum);
                            }
                        },
                        error: function(xhr, status, err) {
                            console.error('Roll fetch error:', status, err);
                        }
                    });
                } else {
                    $('#roll').val('');
            ...
            // Initial load - wait a moment for form to be ready
            setTimeout(fetchNextRoll, 100);
        });
    </script>
</body>
</html>
