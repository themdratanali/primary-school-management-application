...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_teacher'])) {
    ...
    if ($del_id > 0) {
        $del_stmt = $conn->prepare("SELECT photo FROM teachers WHERE id = ?");
        ...
        $conn->query("DELETE FROM teacher_subjects WHERE teacher_id = $del_id");
        $conn->query("DELETE FROM teachers_documents WHERE teacher_id = $del_id");
        $conn->query("DELETE FROM documents WHERE reference_type = 'teacher' AND reference_id = $del_id");

        $del_stmt = $conn->prepare("DELETE FROM teachers WHERE id = ?");
        $del_stmt->bind_param("i", $del_id);
        $del_stmt->execute();
        $del_stmt->close();
    }

    ams_redirect(ams_admin_url('teachers_list'));
    exit;
}
...
$documents = [];
// First try new teachers_documents table
$doc_stmt = $conn->prepare("SELECT file_name, file_path FROM teachers_documents WHERE teacher_id = ? ORDER BY uploaded_at DESC");
...
if (empty($documents)) {
    $old_doc_stmt = $conn->prepare("SELECT file_name, file_path FROM documents WHERE reference_type = 'teacher' AND reference_id = ? ORDER BY uploaded_at DESC");
    ...
    <div class="ams-card-wrapper">
        <div class="profile-card">
            ...
            <div class="profile-avatar">
                <img src="<?= htmlspecialchars($photo) ?>" alt="Teacher Photo" class="profile-avatar-img" onerror="this.onerror=null; this.src='<?= htmlspecialchars(BASE_URL) ?>/uploads/teachers/default-photo.jpg';">
            </div>
            <div class="profile-id">
                <h2 class="profile-name"><?= htmlspecialchars($data['name']) ?></h2>
                <p class="profile-roll"><i class="fas fa-id-badge"></i> <?= htmlspecialchars($data['designation'] ?? 'Teacher') ?></p>
                <div class="profile-badges">
                    <span class="pbadge pbadge-primary"><i class="fas fa-chalkboard-teacher"></i> <?= htmlspecialchars($data['designation'] ?? 'Teacher') ?></span>
                </div>
            </div>
            <div class="profile-actions">
                <a href="<?= ams_admin_url('teacher_profile_edit') ?>?id=<?= $id ?>" class="btn-profile-edit">
                    <i class="fas fa-user-edit"></i> Edit Profile
                </a>
                <form method="post" onsubmit="return confirm('Sure to delete this Teacher profile');" style="margin-top:12px;">
                    <?= ams_csrf_field() ?>
                    <input type="hidden" name="delete_id" value="<?= $id ?>">
                    <button type="submit" name="delete_teacher" class="btn-profile-edit btn-profile-delete">
        ...
        <div class="profile-section">
            ...
            <div class="info-grid">
                ...
                <div class="info-item">
                    <span class="ii-icon"><i class="fas fa-book"></i></span>
                    <span class="ii-text"><span class="ii-label">Subject(s)</span><span class="ii-value"><?= htmlspecialchars($subjects_display) ?></span></span>
                </div>
                <div class="info-item">
        ...
        <div class="profile-section">
            ...
            <div class="info-grid">
                ...
                <div class="info-item">
                    ...
                    <span class="ii-text"><span class="ii-label">Email</span><span class="ii-value"><?= htmlspecialchars($data['email']) ?></span></span>
                </div>
            </div>
        </div>
