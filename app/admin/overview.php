...
// Count students by batch and class
...
$year_admission_students = 0;
$current_year_batch = $conn->query("SELECT * FROM batches WHERE name LIKE '%" . $conn->real_escape_string($current_year) . "%'");
...
if ($results_check && $results_check->num_rows > 0) {
    ...
    while ($result = $all_results->fetch_assoc()) {
        ...
        elseif ($marks >= 70) $gpa_data['A-']++;
        elseif ($marks >= 60) $gpa_data['B']++;
        elseif ($marks >= 50) $gpa_data['C']++;
