...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['file'])) {
    $title = $_POST['title'] ?? '';
    $target_dir = ams_upload_dir('routine');
    ...
    if (move_uploaded_file($file['tmp_name'], $file_path)) {
        $stmt = $conn->prepare("INSERT INTO routine_files (title, filename, file_path, uploaded_by) VALUES (?, ?, ?, ?)");
...
if (isset($_GET['delete'])) {
    ...
    $query = "SELECT file_path FROM routine_files WHERE id = ?";
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        $conn->query("DELETE FROM routine_files WHERE id = $id");
        $success = "File deleted successfully!";
    }
}

// Fetch all files
$files = $conn->query("SELECT * FROM routine_files ORDER BY uploaded_date DESC");
?>
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo BASE_URL; ?>/library/css/manage_routine.css">
</head>
<body>
    <div class="dashboard-title">Manage Routine</div>
    <div class="container">
        <div class="left">
            ...
            <form method="POST" enctype="multipart/form-data">
                <input type="text" name="title" placeholder="e.g., Annual Routine 2026" required>
                ...
                <button type="submit"><i class="fas fa-upload"></i> Upload Routine</button>
            </form>
        </div>
