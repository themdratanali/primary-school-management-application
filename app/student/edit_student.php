...
if (isset($_POST['submit'])) {
  ...
  if (isset($_FILES['photo']) && $_FILES['photo']['error'] == 0) {
    ...
    if (move_uploaded_file($_FILES['photo']['tmp_name'], $newPhoto)) {
      if (!empty($photo) && file_exists($photo) && $photo !== ams_upload_url('students', 'default-photo.jpg')) {
        unlink($photo);
      }
  ...
  if ($stmt->execute()) {
    ...
    header("Location: edit_student?id=$id&updated=1");
    exit;
  } else {
...
<body>
  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <?php
    $photo = (!empty($student['photo']) && file_exists($student['photo'])) ? $student['photo'] : ams_upload_url('students', 'default-photo.jpg');
    ?>
  ...
  <p><a href="student_profile?id=<?= $student['id'] ?>">← Back to Profile</a></p>
